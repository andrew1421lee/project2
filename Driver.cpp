...
bool OptimalCache(std::ifstream &infile);
...
int main(int argc, char* argv[])
{
    ...
    if(infile)
    {
        ...
        if(unlocked)
        {
            LogPrint(INFO, "Super secret replacement unlocked!");
            OptimalCache(infile);
        }
    }
}
...
bool OptimalCache(std::ifstream &infile)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        std::ifstream the_future(infilename);
        ...
        while(infile >> instr >> std::hex >> addr)
        {
            ...
            if(!found)
            {
                ...
                //loop entire file
                unsigned counter = 0;
                unsigned* counters = new unsigned[asses[i]];
                for(int p = 0; p < asses[i]; p++)
                {
                    counters[p] = 0;
                }
                unsigned o_instr;
                ...
                while(the_future >> o_instr >> std::hex >> o_addr)
                {
                    o_addr >>= 5;
                    unsigned o_index = o_addr % (cachesize/ (32 * asses[i]));
                    unsigned o_tag = o_addr >> shifts[i];
                
                    for(int j = 0; j < asses[i]; j++)
                    {
                        if(cache[cindex][j].tag == o_tag)
                        {
                            //next occurence
                ...
                the_future.seekg(accesses - 1, std::ios::beg);

                //infile.seekg(accesses, std::ios::beg);

//                LogPrint(DEBUG, std::to_string(accesses));
                
                //unsigned largest = 0;
                for(int p = 0; p < asses[i]; p++)
                {
                    if(counters[p] > counters[opt])
                    {
                        opt = p;
                    }
                }

                cache[cindex][opt].tag = ctag;
                cache[cindex][opt].last = accesses;
            }
            else
            {
                the_future.seekg(1, std::ios::cur);
            }
